...
description: '모바일에서 모달을 열고 데스크탑으로 크기를 바꾸면 스크롤이 잠기는 버그를 잡았다
...
데이터베이스 테이블을 9개에서 3개로 줄이고 프레임워크를 도입하고 에이전트를 정리하자.

핵심 질문은 "지금 싸게 만들 수 있는가"였다.
...
스크롤 버그에서 기준은 "논리적 상태"가 아니라 "실제 DOM 상태"였다.
...
상태 머신이 과도한 설계에서 기반으로, RAG가 일관성 보장에서 검색 보조로.
