...
description: '이날은 확인과 가정을 구분한 날이었다 삭제 가능 플래그를 믿었는데 실제 환경에서 거짓이었다 포트가 열려 있으면 내 서버라고 가정했는데 다른 작업자의 서버였다 잠금 파일이 오래되면 데몬이 영원히 안 떴다 그리고 컨텍스트 이벤트 주입이 구조적으로 1턴 늦는다는 것도 깨달았다.' date: '2026-07-30'
