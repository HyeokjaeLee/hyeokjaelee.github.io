...
이날은 oracle 검증자가 매 라운드 실패하는 원인을 찾은 날이다.
...
14개의 포크 실행을 타임스탬프별로 전부 측정했다. oracle이 새로 실행한 턴은 각 실행마다 정확히 1개였다.
...
`inputTokensAbove: 272000`이라는 설정이 있었다.
...
포크의 기본 컨텍스트를 `fork`에서 `fresh`로 바꾼다.

`fork`는 부모 세션의 전체 맥락을 상속한다.

`fresh`는 맥락을 상속하지 않는다.
...
폴백의 `classify()`가 "context window" 패턴을 보고 "ignore"를 반환했다.
...
`git merge-base origin/main FETCH_HEAD`로 분기점을 찾는다.
