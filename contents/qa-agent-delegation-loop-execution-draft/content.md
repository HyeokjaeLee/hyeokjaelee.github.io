...
열기, 클릭, 입력, 스크린샷, 스냅샷 같은 기본 동작이 다 의도대로 돌아갔다.
...
마스터가 "다 끝났다"고 해도 Checker가 통과시켜야 진짜 완료다.
...
일주일 전에 JSON에서 SQLite로 바꿨는데, 며칠 쓰다 보니 스키마가 정리가 필요해졌다.
...
근데 한 번 학습한 걸 Fixture에 저장해두면, 다음 실행 때는 그걸 그대로 가져다 쓴다.
