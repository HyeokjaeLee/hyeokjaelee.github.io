...
LLM 에이전트가 대화를 이어가다 보면 컨텍스트 윈도우가 차는데, 이걸 압축해서 더 긴 대화를 유지할 수 있게 돕는다.
...
플랫폼별 바이너리는 GitHub Releases로 배포하고 `postinstall` 스크립트가 알아서 플랫폼에 맞는 바이너리를 내려받는다. macOS arm64, x64, Linux x64, arm64, Windows x64를 다 커버한다.
