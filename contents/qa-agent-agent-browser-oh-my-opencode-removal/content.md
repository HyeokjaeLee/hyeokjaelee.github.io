...
브라우저 세션을 코드 레벨에서 직접 통제할 수 있게 됐다.
...
브라우저를 띄우면, 같은 계정으로 로그인하면서 서로 세션을 밟거나 충돌이 난다.

소유권을 마스터 에이전트 하나로 모으니까
...
브라우저 프로필 디렉토리도 스레드마다

따로 둬서 쿠키랑 로그인 상태가 섞이지 않게 한다.
...
한 슬랙 스레드에서 돌린 QA가 다른 스레드에 영향을 주지 않는다.
