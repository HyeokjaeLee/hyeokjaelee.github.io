...
이번에 대학교 졸업을 앞두면서 Frontend engineer로 커리어를 시작하게 되었다.
...
사실 군대도 군대지만 군대를 전역하고 중간에 빈 시간을 그냥 놀고 싶지는 않았기 때문에 말년휴가를 나와 아르바이트처럼 일할 생각으로 몇 가지 회사들의 면접을 다녔다.
...
![image](https://user-images.githubusercontent.com/71566740/154341538-528f0f77-83fb-4f05-9208-2c8067a180e4.png)
...
금융 관련 기업이다 보니 인터뷰 전 보안서약서를 작성한다.
...
사실 이 글을 쓰고 있는 날짜를 기준으로 오늘 점심에 최종면접 일정이 잡혀있었지만 Toss나 굿닥과 같은 이유로 채용 프로세스를 종료했다.
...
서류 통과 이후 굉장히 빠른 속도로 진행되었고 정말 좋은 채용 프로세스 경험을 제공했다.

## 전체적인 채용 과정에 대한 생각

### 서류

사실 어떤 서류가 좋은 거고 나쁜 건지 나는 아직 잘 모르겠다.

어느 정도 이력서에 경쟁력이 생기면 사실 서류 합격은 걱정하지 않아도 될 것 같다.

개발자의 경우엔 특히 요즘 자유 형식 이력서를 받는 경우가 많기 때문에 나는 이력서이자 포트폴리오인 사이트를 하나 만들어 모든 기업에 지원했다.

그냥 자신을 잘 나타낼 수 있는 게 좋은 이력서라고 생각한다.

### 코딩 테스트

아무래도 개발자의 실력을 보지 않고 함께 일할수는 없기 때문에 이해는 가지만 사실 나는 일반 알고리즘 코딩 테스트가 썩 반갑지는 않다.

그래도 과제형 코딩 테스트 처럼 많은 시간을 들일 필요없이 2시간~6시간 정도로 평가를 마칠수 있으니 기업과 지원자에게는 가장 편리한 실력 검증 수단임에는 틀림없다.
...
아무래도 퇴사한 직장 동료분들께 연락을 하는 것이다 보니 부담스러웠다.

하지만 함께 일해본 동료들이 나를 평가하는 것만큼 기업 입장에서 지원자를 검증하기 좋은 수단은 없다고 생각하기 때문에 이해할 수 있다.

지원한 회사가 한두 곳이 아니었기 때문에 전 직장 동료분들이 많이 귀찮으셨을 것 같다..
...
아무래도 각 회사마다 개발자에게 요구하는 역량이 다르다 보니 인터뷰 질문들도 재각각이고(일반적인 CS, JS의 작동원리, 실무 등) 한 번도 코딩 테스트를 공부해본 적이 없다 보니(그 흔한 백준 계정도 없음..) 코딩 테스트도 실제 채용 과정에서 부딪히면서 했다. (상놈식 공부법)
