...
일단 Gatsby에 공유되어있는 템플릿 중에 정말 괜찮은 디자인이 많아서 템플릿 중 하나를 수정해서 사용하려 했습니다.

하지만 블로그 제작을 목적으로 하는 템플릿은 맘에 드는 게 없어서 다른 용도의 템플릿 중에 맘에 드는 것을 찾아서 몇 가지 기능을 추가해서 사용하려 했는데 기능을 추가해 가면서 다음과 같은 문제점이 있어서 그냥 템플릿 없이 기본 Gatsby 설정으로 새 블로그를 만들게 되었습니다.
