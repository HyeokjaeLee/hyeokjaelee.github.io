...
`midRun`, `afterRun`, `nextTurn`이라는 이름을 썼다.
...
`thisStep`, `nextStep`, `idle`, `nextUserRequest` 4개 값으로 정리했다.

근데 `idle`에 문제가 있었다.

"쉬고 있을 때"인데, "쉴 때 깨워라"와 "쉴 때 깨우지 마라"를 구분할 수 없었다.

별도의 `triggerTurn` 축을 추가해서 해결하려 했다.
...
`triggerTurn`을 유지하면서 4개 값을 개선했다.
...
`idle`을 둘로 쪼개면, `triggerTurn` 축이 필요 없어진다.

`idleWithWakeup`이 자체적으로 "깨운다"를 포함하고 `idle`이 자체적으로 "안 깨운다"를 포함한다.

두 축이 하나의 값으로 합쳐진다.

다른 값들(`thisStep`, `nextStep`, `nextUserRequest`)은 깨움 여부가 자명하다.

`thisStep`은 당연히 깨운다.

`nextUserRequest`는 당연히 안 깨운다.

깨움 여부가 모호한 건 `idle`뿐이었다.
...
기존의 `triggerTurn: true`는 `idleWithWakeup`으로, `triggerTurn: false`는 `idle` 또는 `nextStep`으로 변환됐다.
...
Bun에서 code-index 확장이 치명적으로 실패한다. code-index가 `node:sqlite`를 import하는데, Bun은 이를 지원하지 않는다.

`node:sqlite`를 `bun:sqlite`로 포팅할 수는 있지만 그다음 `node-llama-cpp`라는 네이티브 애드온이 또다시 벽이다.

네이티브 `.node` 파일은 Bun에서 로드할 수 없다.
