...
description: '메시지 배달 API를 설계했다 언제 배달할지와 깨울지 말지를 하나의 필드로 표현하고 싶었다 네 번 실패했다 다섯 번째에서 idle을 둘로 쪼개는 게 정답이었다 그리고 Bun 런타임으로의 마이그레이션을 측정 데이터로 거절한 것도 있었다.' date: '2026-07-28'
