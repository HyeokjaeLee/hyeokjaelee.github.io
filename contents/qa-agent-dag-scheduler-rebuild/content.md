...
며칠째 이어지던 QA Agent 구조 개편에서 실행 모델을 갈아없은 날이었다.
...
그러니까 실패가 감지되면 하위 케이스는 재시도하지 않고 바로 차단 표시를 한다.
...
`executionGroups`에서 `cases`로 바꾸는 과정에서 남은 잔재를 정리했다.

Executor 타임아웃 구조를 다듬었다.

막힌 케이스 원인 분석 로직을 보강했다.

그리고 `source-lookups`라는 기능을 완전히 걷어냈다.
