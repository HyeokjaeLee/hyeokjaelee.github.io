...
description: '구조화된 출력을 위해 JSON 스키마를 보냈더니 NVIDIA 모델이 40초마다 타임아웃 났다 OpenAI 호환이라는 말을 믿으면 안 되는 이유.

그리고 연속성 원장을 전체 스택에 구현했다.'
...
가장 이식성 높은 방법은 일반 텍스트로 받고 직접 JSON을 파싱하고 검증하는 거다.
