...
QA Agent planning 파이프라인을 갈아없은 날이었다.
...
병렬로 돌리니까 전체 시간도 줄고 각 호출은 작으니까 안정적이다.
...
그러니까 이제 스킬이 "어떻게 관리자로 로그인하는가"라는 행동 지식이 됐다.
...
한 번에 전부 하려다 압도되는 문제를, 작게 쪼개서 각자 처리하고 합치는 방식으로 해결했다.
