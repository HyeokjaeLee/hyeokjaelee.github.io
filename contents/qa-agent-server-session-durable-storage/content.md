...
결과를 받아보는 사람이 스레드를 열어봐야 뭔지 알 수 있으니까
...
> 다음 날에 이 Lane 세션 분리 원칙을 확정했다.

Lane은 절대 세션을 공유하지 않는다.
...
하나는 git이 이 런타임 데이터를 계속 트래킹하니까

저장소가 더러워지고 머지 충돌이 났다.

다른 하나는 더 심각했는데, 저장소를 클린하게 다시 받으면 학습한 지식이 싹 날아간다는 거다.

한참 돌려서 쌓은 실행 지식이 깃 클론 한 번이면 사라지니까
...
성공 케이스의 실행 지식을 스킬로 쌓고 다음

실행 때 Planner가 조회해서 쓰는 구조다.
