...
명시적인 any 금지, 안전하지 않은 할당이랑 멤버 접근 금지, 부동 프로미스 금지, null이 아님 단언 금지.

이런 규칙들이 타입 안전성을 강제한다.
...
에디터에서도 린트가 돌고 빌드도 제대로 된다.

## 검증

린트를 도입하고 전부 통과하는지 확인했다.

타입체크 에러 0개, ESLint 에러 0개 경고 0개, 테스트 82개 전부 통과, Prettier 포맷 검사도 전부 통과.

게이트를 통과하는 코드만 들어가게 만든 거다.

> 이 결정은 약 한 달 뒤에 검증됐다.

잠깐 Biome으로 바꿨다가 다시 ESLint랑 Prettier로 돌아온 거다.
