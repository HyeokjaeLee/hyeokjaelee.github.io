...
공유 파일은 못 건드리니까 컴포넌트에서 강제로 우선하게 만든 거다. important를 쓰는 건 별로 안 좋지만 공유 파일을 못 건드리면 어쩔 수 없다.
...
규칙 만들기, 알림 규칙 만들기, 학습 한계 정규화, 접근 제한 해결.

각각 하나의 일만 하는 작은 함수로 분리했다.
