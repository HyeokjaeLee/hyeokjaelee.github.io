...
67.5메가바이트의 세션 기록에서 한 번도 발동하지 않았다.
...
확장 디렉토리를 밖으로 옮기면, 컴파일/패키지/런타임 세 단계가 한 번에 끊긴다. disable 플래그가 필요 없다.
...
"하나만 고정하면 캐시가 따뜻해지지만 단일 실패 지점이 된다." "여러 개 쓰면 가용성은 좋지만 캐시가 안 따뜻해진다." 이 딜레마를 `order` 리스트로 해결한다.

정상일 때는 집중, 장애일 때만 분산.

## chartPreviousClose를 잘못 읽었다

야후 파이낸스의 `chartPreviousClose`를 "전 거래일 종가"로 읽었다.
...
정확한 계산은 `closes[-1]` vs `closes[-2]`로 단일일을 구하고 `chartPreviousClose`는 누적 범위 계산에만 쓴다.
...
그리고 `chartPreviousClose`를 필드 이름으로 추론했다.
