...
속도 제한, 인증 에러, 할당량 소진, 서버 일시적 장애.

근데 실패 이유가 뭐든 간에, 에이전트가 멈추면 처음부터 다시 해야 한다.
...
이런 건 잠시 기다렸다가 재시도하면 될 수도 있다.

그러니까

지수 백오프로 재시도를 먼저 해보고 그래도 안 되면 넘어간다.
...
이후 몇 주에 걸쳐 포크 기반 구조랑 상태 머신 구조로 더 발전했다.
...
영구 에러인지 일시 에러인지, 어느 모델이 실패했는지, 쿨다운이 끝났는지.

이런 결정이 틀리면 폴백이 엉뚱하게 동작한다.
...
에러를 영구랑 일시로 나누고 모델 단위로 추적하고 시간 제한 쿨다운을 두고 컨텍스트가 차면 큰 모델로 넘어가고.

이 네 가지 결정이 플러그인의 뼈대가 됐다.
