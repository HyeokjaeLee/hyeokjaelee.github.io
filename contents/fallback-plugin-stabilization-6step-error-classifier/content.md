...
description: '며칠간 만든 폴백 플러그인을 안정화했다
...
쿨다운 중인 모델한테 또 폴백을 시도하는 경우가 있었다.
...
폴백이 왜 발생했는지, 어느 모델에서 어느 모델로 넘어갔는지, 에러가 뭐였는지.

이런 정보가 구조화되어 있으니까, 문제가 생겼을 때 원인을 빨리 찾을 수 있다.
...
나중에 "이게 왜 이렇게 되어 있지" 궁금할 때, 기록이 있으면 바로 알 수 있다.
...
> 이 기록은 며칠 뒤에 동시성 버그를 잡을 때 큰 도움이 됐다.

변수 중복 선언이 모든 이벤트 핸들러를 깨뜨리는 버그, 컨텍스트 압축 타이밍 경쟁 조건 같은 문제가 이후에 나왔다.

기록이 있으니까

어느 시점에 어떤 변화가 들어갔는지 추적할 수 있었다.
