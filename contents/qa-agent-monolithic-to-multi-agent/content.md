...
그 전까지 QA Agent는 `runQaJob`이라는 함수 하나가 전부를 처리했다.

계획 세우기, 실행, 리포팅까지 순차적으로 다 돌렸다.
...
막힌 게 너무 많으면 무작정 다 동시에 돌리지 않고 제한을 두는 식이다.
...
`source-skills`는 원본, `skills`는 런타임에 수정된 버전.

이렇게 분리하니까 원본을 잃어버리지 않으면서 런타임 수정을 할 수 있었다.
...
그래서 CREATE-only 모드로 바꿨다. idle이 발생할 때마다 새 타임스탬프 노트를 만든다.
