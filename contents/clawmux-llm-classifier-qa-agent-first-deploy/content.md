...
처음엔 키워드 점수 계산기였고 이날 LLM 분류기로 바꿨다.

다음

날에는 로컬 임베딩 분류기를 시도했다가 한계에 부딪혀서 결국 다시 싼 LLM 한 글자 방식으로 돌아왔다.
...
크래시가 나면 전체 QA 실행이 멈추니까

개별 에러가 전파되지 않게 막는 게 중요했다.
...
ClawMux 분류기의 핵심 교훈은 가벼운 LLM이 내놓는 한 글자가 생각보다
...
로컬에서 잘 돌아가는 거랑 실제 환경에서 안정적으로 돌아가는 건 다른 문제인데 배포하고 크래시를 잡으니까
