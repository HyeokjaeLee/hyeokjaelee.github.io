...
이날 나는 pi 확장이 정보를 주입하는 3가지 경로를 정리한 날이다.
...
`display: false`를 설정하면 TUI에서 박스가 사라진다.

"숨겼다"고 생각한다.

근데 모델은 여전히 읽고 있다.

`display: false`는 TUI 박스를 지울 뿐, 모델 컨텍스트에서 제외하지 않는다.

변환 함수가 여전히 `role: user`로 매핑해서 모델에 보낸다.

"화면에 안 보이니까

모델도 안 보겠지"라는 가정이 틀렸다.
...
하나는 codebase_search를 권장하고 다른 하나는 제한했다.
...
기존에는 `disable-model-invocation` 플래그로 시스템 프롬프트에서 숨기기만 했다.
