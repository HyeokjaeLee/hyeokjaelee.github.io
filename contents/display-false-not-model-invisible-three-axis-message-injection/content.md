...
description: '확장 메시지를 화면에서 숨겼다 근데 모델은 여전히 읽고 있었다.

display false는 TUI 박스를 지울 뿐, 모델 컨텍스트에서는 제외되지 않는다 이걸 깨닫고 메시지 주입 3축을 정리했다 그리고 같은 날 두 세션이 같은 프롬프트를 반대 방향으로 수정한 것도 발견했다.'
