...
description: '코드 인덱싱 데몬이 응답하지 않았다 원인은 onnxruntime 동기 호출이 이벤트 루프를 얼린 거였다 근데 얼어붙은 루프가 자기 자신을 감시할 수는 없다 별도 프로세스가 필요했다 그리고 통과한 테스트가 실제 규모에서는 의미가 없었다.' date: '2026-07-21'
