...
> 이 잠금 모달이랑 툴팁은 나중에 공통 패키지로 승격됐다.

여러 앱에서 같은 잠금 UI를 쓰게 된 거다.
...
디렉토리 목록은 구조적으로 출력이 길어서 압축이 잘 된다. git 상태는 약 64%가 줄었다. grep은 약 44%가 줄었다. grep은 이미 출력이 꽤 조밀해서 압축 효율이 상대적으로 낮았다.
