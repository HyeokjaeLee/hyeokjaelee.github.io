...
description: 'QA 에이전트가 요약된 값을 받아서 잘못 통과시킨 일로 검증 규칙을 네 번 다듬었다
...
이날 나는 며칠간의 변경사항을 하나로 묶어서 푸시한 날이다.
